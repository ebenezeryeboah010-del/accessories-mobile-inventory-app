...
export default function App() {
  ...
  const handleSellUnit = async (item: InventoryItem) => {
    if (!token) return;
    if (item.quantity <= 0) {
      toast(`Cannot sell "${item.name}". Quantity units in stock is 0.`);
      return;
    }

    const newQuantity = item.quantity - 1;
    const newSoldQuantity = item.soldQuantity + 1;

    // Optimistic UI update so all financial report states recalculate instantly
    setInventory((prev) =>
      prev.map((i) => (i.id === item.id ? { ...i, quantity: newQuantity, soldQuantity: newSoldQuantity } : i))
    );
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json();
        throw new Error(d.error || "Sale record transaction failed");
      }

      toast(`Sale recorded for "${item.name}"! Quantity in stock regulated to ${newQuantity}, sold units: ${newSoldQuantity}.`);
      fetchAllData(token);
    } catch (err: any) {
      toast(err.message);
      fetchAllData(token);
  ...
  const handleDeleteItem = (itemOrId: InventoryItem | string) => {
    const item = typeof itemOrId === "string" ? inventory.find((i) => i.id === itemOrId) : itemOrId;
    if (item) {
      setItemToDelete(item);
    } else if (typeof itemOrId === "string") {
      setItemToDelete({ id: itemOrId, name: "Stock Item", sku: itemOrId, categoryId: "", type: "", quantity: 0, soldQuantity: 0, costPrice: 0, sellingPrice: 0 } as InventoryItem);
    }
  };
  ...
  const confirmDeleteItem = async () => {
    ...
    // Optimistic deletion so financial reports update immediately
    ...
    try {
      ...
      toast(`Stock item "${itemName}" deleted permanently. Financial metrics automatically recalculated.`);
      fetchAllData(token);
    } catch (err: any) {
      toast(err.message);
      fetchAllData(token);
  ...
  const handleDeleteCategory = (catOrId: Category | string) => {
    const cat = typeof catOrId === "string" ? categories.find((c) => c.id === catOrId) : catOrId;
    if (cat) {
      setCategoryToDelete(cat);
    } else if (typeof catOrId === "string") {
      setCategoryToDelete({ id: catOrId, name: "Selected Category", group: "" } as Category);
    }
  };
  ...
  const confirmDeleteCategory = async () => {
    ...
    try {
      ...
      toast(`Category "${catName}" deleted gracefully. Items reassigned to Uncategorized.`);
      fetchAllData(token);
    } catch (e: any) {
      toast(e.message);
    }
  ...
  // ADVANCED REPORT CSV EXPORT HELPER (INCLUDES GENERAL PROFIT & FINANCIAL SUMMARY)
  const exportAdvancedReportCsv = () => {
    const liveStockSplit = {
      currentStockCost: inventory.reduce((sum, item) => sum + item.quantity * item.costPrice, 0),
      soldStockCost: inventory.reduce((sum, item) => sum + item.soldQuantity * item.costPrice, 0),
      totalInventoryCost: inventory.reduce((sum, item) => sum + (item.quantity + item.soldQuantity) * item.costPrice, 0),
      currentStockRetailValue: inventory.reduce((sum, item) => sum + item.quantity * item.sellingPrice, 0),
      soldRetailValue: inventory.reduce((sum, item) => sum + item.soldQuantity * item.sellingPrice, 0),
    };
    const currentSplit = stockSplit || liveStockSplit;
    const generalProfit = currentSplit.soldRetailValue - currentSplit.soldStockCost;

    const rows: string[] = [];

    rows.push(`"ADVANCED FINANCIAL & PROFIT REPORT - YEBOAH ACCESSORIES"`);
    rows.push(`"Generated At: ${new Date().toLocaleString()}"`);
    rows.push(`"Currency: ${settings.currency || "USD"}"`);
    rows.push("");

    // Section 1: Executive Financial Summary Metrics
    rows.push(`"EXECUTIVE METRICS OVERVIEW"`);
    rows.push(`"Metric","Value (${settings.currency || "USD"})"`);
    rows.push(`"Total Asset Initial Cost (Remaining + Sold Cost)","${currentSplit.totalInventoryCost.toFixed(2)}"`);
    rows.push(`"Remaining Stock Cost Basis","${currentSplit.currentStockCost.toFixed(2)}"`);
    rows.push(`"Sold Goods Cost Basis","${currentSplit.soldStockCost.toFixed(2)}"`);
    rows.push(`"Remaining Retail Selling Value","${currentSplit.currentStockRetailValue.toFixed(2)}"`);
    rows.push(`"Total Sales Proceeds (Sold Retail Value)","${currentSplit.soldRetailValue.toFixed(2)}"`);
    rows.push(`"General Profit of Goods Sold","${generalProfit.toFixed(2)}"`);
    rows.push("");

    // Section 2: Category Financial Breakdown
    rows.push(`"CATEGORY FINANCIAL SUMMARY"`);
    const catHeaders = [
      "Category Name",
      "Type Group",
      "Distinct SKUs",
      ...
      "Stock Asset Cost",
      "Sales Proceeds",
      "Remaining Retail Value",
      "Category General Profit"
    ];
    rows.push(catHeaders.map(h => `"${h}"`).join(","));

    const activeCatReport = summaryReport.length > 0 ? summaryReport : categories.map(cat => {
      ...
      return {
        ...
        totalSoldQuantity: catItems.reduce((sum, i) => sum + i.soldQuantity, 0),
        inventoryCostValue: catItems.reduce((sum, i) => sum + i.quantity * i.costPrice, 0),
        soldValue: catItems.reduce((sum, i) => sum + i.soldQuantity * i.sellingPrice, 0),
        remainingStockValue: catItems.reduce((sum, i) => sum + i.quantity * i.sellingPrice, 0),
        totalSoldCostValue: catItems.reduce((sum, i) => sum + i.soldQuantity * i.costPrice, 0),
      };
    });

    let grandSkus = 0, grandInStock = 0, grandSoldUnits = 0, grandSoldCost = 0, grandAssetCost = 0, grandProceeds = 0, grandRemainingRetail = 0, grandNetProfit = 0;

    activeCatReport.forEach(r => {
      const soldCost = r.totalSoldCostValue || 0;
      const profit = r.soldValue - soldCost;

      grandSkus += r.itemCount || 0;
      grandInStock += r.totalQuantity || 0;
      grandSoldUnits += r.totalSoldQuantity || 0;
      grandSoldCost += soldCost;
      grandAssetCost += r.inventoryCostValue || 0;
      grandProceeds += r.soldValue || 0;
      grandRemainingRetail += r.remainingStockValue || 0;
      grandNetProfit += profit;

      rows.push([
        r.categoryName,
        r.categoryGroup || "General",
        r.itemCount,
        r.totalQuantity,
        r.totalSoldQuantity,
        soldCost.toFixed(2),
        r.inventoryCostValue.toFixed(2),
        r.soldValue.toFixed(2),
        r.remainingStockValue.toFixed(2),
        profit.toFixed(2)
      ].map(val => `"${String(val ?? "").replace(/"/g, '""')}"`).join(","));
    });
    ...
    rows.push([
      ...
      grandInStock,
      grandSoldUnits,
      grandSoldCost.toFixed(2),
      grandAssetCost.toFixed(2),
      grandProceeds.toFixed(2),
      grandRemainingRetail.toFixed(2),
      grandNetProfit.toFixed(2)
    ].map(val => `"${String(val ?? "").replace(/"/g, '""')}"`).join(","));

    rows.push("");

    // Section 3: Low Stock Alerts Audit
    if (lowStockReport.length > 0) {
      rows.push(`"LOW STOCK EXCEPTIONS AUDIT"`);
      rows.push(`"Product Name","SKU","Category","Current Units Left","Low Stock Threshold"`);
      lowStockReport.forEach(item => {
        const cat = categories.find(c => c.id === item.categoryId);
        const thresh = cat && cat.lowStockThreshold !== undefined && cat.lowStockThreshold !== null ? cat.lowStockThreshold : (settings.lowStockThreshold || 8);
        rows.push([
          item.name,
          item.sku,
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `Yeboah_Accessories_Advanced_Financial_Report_${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
    URL.revokeObjectURL(url);
    toast("Advanced Financial & Profit CSV Report downloaded successfully.");
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex transition-colors duration-200">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-8 flex-1 overflow-y-auto max-w-[1300px] w-full mx-auto">
                              ...
                              {me?.role !== "Staff" && (
                                <td className="p-4 pr-6 text-right">
                                  <div className="relative inline-flex gap-1 justify-end bg-transparent">
                                    <button
                                      onClick={() => handleSellUnit(item)}
                                      className="p-1.5 text-emerald-600 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-slate-800 rounded-lg shrink-0 flex items-center gap-1 font-bold text-xs px-2.5 border border-emerald-500/30 transition-all cursor-pointer"
                                      title="Record sale of 1 unit (Automatically regulates units in stock and pre-sold units)"
                                    >
          ...
          {activeTab === "reports" && (() => {
            const liveStockSplit: StockSplitReport = {
              currentStockCost: inventory.reduce((sum, item) => sum + item.quantity * item.costPrice, 0),
              soldStockCost: inventory.reduce((sum, item) => sum + item.soldQuantity * item.costPrice, 0),
              totalInventoryCost: inventory.reduce((sum, item) => sum + (item.quantity + item.soldQuantity) * item.costPrice, 0),
              currentStockRetailValue: inventory.reduce((sum, item) => sum + item.quantity * item.sellingPrice, 0),
            ...
            const liveCategoryReport: CategoryReport[] = categories.map((cat) => {
              const catItems = inventory.filter((i) => i.categoryId === cat.id);
              let totalQuantity = 0;
            ...
            const activeSummaryReport = summaryReport.length > 0 ? summaryReport : liveCategoryReport;

            const grandTotalSkus = activeSummaryReport.reduce((acc, r) => acc + (r.itemCount || 0), 0);
            const grandTotalInStock = activeSummaryReport.reduce((acc, r) => acc + (r.totalQuantity || 0), 0);
            const grandTotalSoldQuantity = activeSummaryReport.reduce((acc, r) => acc + (r.totalSoldQuantity || 0), 0);
            const grandTotalSoldCost = activeSummaryReport.reduce((acc, r) => acc + (r.totalSoldCostValue || 0), 0);
            const grandTotalAssetCost = activeSummaryReport.reduce((acc, r) => acc + (r.inventoryCostValue || 0), 0);
            const grandTotalSalesProceeds = activeSummaryReport.reduce((acc, r) => acc + (r.soldValue || 0), 0);
            const grandTotalRemainingRetail = activeSummaryReport.reduce((acc, r) => acc + (r.remainingStockValue || 0), 0);
            const grandTotalNetProfit = grandTotalSalesProceeds - grandTotalSoldCost;

            return (
              <div className="space-y-8 bg-transparent">
                {/* Advanced Report Header Bar with CSV Export */}
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200/50 dark:border-slate-850 shadow-sm">
                  <div>
                    <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                      Advanced Financial & Profit Report
                    </h2>
                    <p className="text-xs text-slate-500 font-medium mt-1">
                      Real-time valuation of assets, sold goods cost basis, sales proceeds, profit margins, and inventory audit.
                    </p>
                  </div>
                  <button
                    onClick={exportAdvancedReportCsv}
                    className="bg-emerald-600 hover:bg-emerald-500 text-white h-[44px] px-5 rounded-lg font-bold text-sm shadow flex items-center gap-2 transition-all cursor-pointer shrink-0"
                  >
                    <FileSpreadsheet className="w-4 h-4" />
                    Export Advanced Report CSV
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200/50 dark:border-slate-850 shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-slate-900 dark:text-white mt-1.5">
                      {fmtMoney(currentSplit.totalInventoryCost)}
                    </h3>
                    <p className="text-[11px] text-slate-500 mt-2">
                      Aggregate cost value of all stocked items (both remaining and sold): <br />
                      Remaining Cost: {fmtMoney(currentSplit.currentStockCost)} + <br />
                      Sold Cost Basis: {fmtMoney(currentSplit.soldStockCost)}
                    </p>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200/50 dark:border-slate-850 shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-emerald-500 mt-1.5">
                      {fmtMoney(currentSplit.currentStockRetailValue)}
                    </h3>
                    <p className="text-[11px] text-slate-500 mt-2">
                  ...
                  <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200/50 dark:border-slate-850 shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1.5">
                      {fmtMoney(currentSplit.soldRetailValue - currentSplit.soldStockCost)}
                    </h3>
                    <p className="text-[11px] text-slate-500 mt-2">
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200/50 dark:border-slate-850 shadow-sm">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[1000px] text-sm md:text-sm">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-850/50 font-bold uppercase tracking-wider text-[11px] text-slate-500 border-b border-slate-200 dark:border-slate-850">
                          ...
                          <th className="p-3 text-center">Category General Profit</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                        {activeSummaryReport.length === 0 ? (
                          <tr>
                          ...
                          <>
                            {activeSummaryReport.map((rep) => {
                              const catProfit = rep.soldValue - (rep.totalSoldCostValue || 0);
                              return (
                            ...
                            <tr className="bg-slate-100/80 dark:bg-slate-800/80 font-black border-t-2 border-slate-300 dark:border-slate-700 text-slate-950 dark:text-white">
                              ...
                              <td className="p-3 text-center font-mono text-emerald-600 dark:text-emerald-400 font-extrabold">
                                {fmtMoney(grandTotalNetProfit)}
                              </td>
                            </tr>
                          </>
                        )}
      ...
      {itemToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-xl space-y-4">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-850 rounded-xl border border-slate-200/60 dark:border-slate-800 text-sm space-y-1">
              <p className="font-bold text-slate-900 dark:text-white">{itemToDelete.name}</p>
              <p className="text-xs text-slate-500 font-mono">SKU: {itemToDelete.sku}</p>
              <p className="text-xs text-slate-500">
                Units in Stock: <span className="font-bold text-slate-700 dark:text-slate-300">{itemToDelete.quantity}</span> | Sold Units: <span className="font-bold text-slate-700 dark:text-slate-300">{itemToDelete.soldQuantity}</span>
              </p>
            </div>

            <p className="text-xs text-slate-500">
              Deleting this item will permanently remove it from inventory. All financial metrics (Total Asset Initial Cost, Remaining Retail Value, and General Profit) will be automatically recalculated.
            </p>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-lg bg-red-600 hover:bg-red-500 text-white text-xs font-extrabold shadow-sm transition-all cursor-pointer"
              >
                Delete Permanently
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-xl space-y-4">
            <div className="flex items-center gap-3 text-red-600 dark:text-red-400">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Items under this category will fallback to 'Uncategorized'.</p>
              </div>
            </div>

            <div className="p-4 bg-slate-50 dark:bg-slate-850 rounded-xl border border-slate-200/60 dark:border-slate-800 text-sm">
              <p className="font-bold text-slate-900 dark:text-white">{categoryToDelete.name}</p>
              <p className="text-xs text-slate-500">Group: {categoryToDelete.group || "General"}</p>
            </div>
